...
│   │   ├── blazeface_cnn_backend.py  # the real backend -- BlazeFace + 2 tiny CNNs
...
│   ├── link/
│   │   ├── icd.py                 # wire format, byte-identical to drowsyguard_vcs/src/icd/icd.h
│   │   ├── crc8.py                # same CRC-8, same test vectors as crc8.c
...
└── tools/replay_corpus.py        # CLI: replay a JSONL corpus through the real pipeline
```
...
- **`link/icd.py` + `crc8.py`** — byte-exact round-trips against hand-computed payloads, CRC
  ...
  vectors as `drowsyguard_vcs/src/icd/crc8.c`** — see `link/README.md` for why that duplication
  exists and what closing it properly (specs 02 DEV-002) would look like.
- **`tools/replay_corpus.py`** works end to end: feeding a synthetic closure event through it
  correctly produces `L0 → L1 (at 800ms) → L2 (at 1500ms)`, i.e. the whole domains→fusion chain
  agrees with the spec on a case it wasn't unit-tested against directly.
...
- **Head pose (D1's yaw/pitch) is not implemented.** `blazeface_cnn_backend.py` always returns
  `yaw_deg=None, pitch_deg=None` — BlazeFace's 6 keypoints alone don't give a validated pose
  estimate without solvePnP + a 3D face model, which was never built. **D1 (distraction) currently
  detects nothing on the live pipeline** — its domain logic is complete and tested, it just never
  receives real evidence. See the docstring in `blazeface_cnn_backend.py`.
...
- **The AP↔RT transport does not exist.** `link/ap_rt_transport.py` only has `NullTransport`
  ...
  feature above.
- **MAR-based yawn detection (`d2_yawn.mar_open` in thresholds.yaml) is unused.** The implemented
  prototype classifies mouth state with a CNN, not landmark geometry. See the note on `mar_open` in
  `config/thresholds.yaml` and the module docstring in `domains/d2_yawn.py`.
