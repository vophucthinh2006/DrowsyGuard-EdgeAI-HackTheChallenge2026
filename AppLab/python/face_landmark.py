"""
Wrapper around face_landmarks_detector.tflite.

Input:  [1, 256, 256, 3] float32, values in [0, 1]
Output:
  Identity   [1,1,1,1434] -> 478 landmarks (x, y, z); x/y are in pixel
                              coordinates of the 256x256 crop, z is
                              relative depth.
  Identity_1 [1,1,1,1]    -> "is this a face" logit (apply sigmoid)
  Identity_2 [1,1]        -> auxiliary branch, unused here.
"""
import cv2
import numpy as np
...
class FaceLandmarkDetector:
    ...
    def _expand_bbox(bbox, frame_shape, scale=1.6):
        """Expands the bbox from the face detector to cover the whole face
        plus margin (mirrors FaceLandmarksFromPoseCalculator / the
        rect_transformation step in the original graph)."""
    ...
    def predict(self, frame_rgb, bbox):
        """Returns (landmarks Nx3 in original pixel coords, face_score),
        or (None, score) if the crop is empty."""
        ...
        # map from the 256x256 crop space back to the original frame's pixels
        landmarks = raw.copy()
    ...
    def landmarks_to_bbox(landmarks, margin=0.25):
        """Derives the bbox for the next frame from the current frame's
        landmarks - used when tracking is enabled (skipping
        face_detector.tflite on most frames)."""
