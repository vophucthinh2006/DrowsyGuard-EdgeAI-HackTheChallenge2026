...
try:
    # Priority 1: latest LiteRT runtime from Google (ai-edge-litert)
    import ai_edge_litert.interpreter as tflite
except ImportError:
    try:
        # Priority 2: older tflite_runtime
        import tflite_runtime.interpreter as tflite
    except ImportError:
        # Priority 3: fall back to full TensorFlow (only needed on Colab/PC dev)
        import tensorflow.lite as tflite

# Default model path used inside this module
...
class ObstructionDetector:
    def __init__(
        ...
        """
        Initializes the TFLite model that checks for face obstructions
        (mask / sunglasses).
        """
        ...
        if not os.path.exists(self.model_path):
            raise FileNotFoundError(
                ...
                f"Please check the 'models/' folder."
            )
    ...
    def preprocess(self, face_crop: np.ndarray) -> np.ndarray:
        """
        Preprocesses the face crop image (BGR -> RGB -> Resize -> add batch
        dimension).
        """
    ...
    def predict(self, face_crop: np.ndarray) -> dict:
        """
        Predicts mask / sunglasses presence from a cropped face image (BGR).
        """
