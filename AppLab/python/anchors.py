"""
Generates anchor boxes for face_detector.tflite (BlazeFace short-range).

This is the "SsdAnchorsCalculator" algorithm that MediaPipe uses internally
(configured in face_detection_short_range.pbtxt). When you run the .task
bundle through the official Tasks API, this anchor-generation step is
hidden inside the compiled C++ graph. Here it is reimplemented in plain
Python so the whole pipeline stays under our control.

Parameters below match the original face_detection_short_range config:
  num_layers = 4
  min_scale = 0.1484375
  max_scale = 0.75
  input_size = 128x128
...
Result: exactly 896 anchors, matching the model's output shape
[1, 896, 16] / [1, 896, 1].
"""
import math
import numpy as np
...
if __name__ == "__main__":
    a = generate_anchors()
    print("Number of anchors generated:", a.shape)  # expected (896, 4)
